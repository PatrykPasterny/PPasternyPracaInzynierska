...
class ff_parser_1d(object):
    ...
    def __decode_data(self):
        ...
        data_errors = []
        bin_edges = []
        bin_edges.append( float( self.__data[0][ bins[0] ] ) )
        for line in self.__data:
            bin_edges.append(   float( line[ bins[1] ] ) )
            data_points.append( float( line[ data[0] ] ) )
            data_errors.append( float( line[ data[0] ] ) * ( float( line[ data[1] ] )/100. ) )
        
        self.__histogram[ 'BIN_EDGES' ] = bin_edges
...
class plot_1d(object):
    ...
    def __plot_1d(self):
        ...
        self.__histo =  TH1F(name, name, int(bins), bin_edges)
